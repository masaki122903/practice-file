...
#通常クラス名はキャメル(ラクダ)ケースで命名する（単語頭が大文字）
#C言語なら関数はスネークケース（_で結ぶ）
...
#クラス変数は全てのインスタンス間で共有される変数(クラス変数は全部で共有されてしまう)
